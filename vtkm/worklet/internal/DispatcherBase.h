...
VTKM_NEVER_EXPORT auto GetArg(std::integral_constant<vtkm::IdComponent, I>, T0&&, Ts&&... as)
{
  ...
  return GetArg(std::integral_constant<vtkm::IdComponent, I - 1>{},
                std::forward<Ts>(as)...);
}
/// @}
                                       ...
                                       ControlObjectType&& controlObject)
{
  ...
  vtkm::cont::arg::Transport<TransportTag, vtkm::internal::remove_pointer_and_decay<ControlObjectType>, Device> transport;
...
protected:
  ...
  VTKM_NEVER_EXPORT void BasicInvoke(
    InputRangeType inputRange, ControlObjectTypes&&... controlObjects) const
  {
    bool success =
      vtkm::cont::TryExecuteOnDevice(this->Device,
                                     [&](auto device){
      this->InvokeTransportSchedule(inputRange, device, std::forward<ControlObjectTypes>(controlObjects)...);
      return true;
    });
    if (!success)
    {
  ...
  VTKM_NEVER_EXPORT void BasicInvoke(
    vtkm::Id2 inputRange, ControlObjectTypes&&... controlObjects) const
  {
  ...
  VTKM_NEVER_EXPORT void ConvertBasicInvoke(const Invocation& invocation,
                                            const InputRangeType& inputRange,
                                            std::integer_sequence<vtkm::IdComponent, Indices...>) const
  {
    this->BasicInvoke(
      inputRange, vtkm::internal::ParameterGet<Indices + 1>(invocation.Parameters)...);
  }
  ...
  {
    this->ConvertBasicInvoke(invocation, numInstances,
  ...
  {
    this->ConvertBasicInvoke(invocation, dimensions,
                             typename vtkmstd::make_integer_sequence<vtkm::IdComponent,
                             Invocation::ParameterInterface::ARITY>{});
  }
