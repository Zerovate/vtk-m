...
template <typename ExecSigReturn, typename ControlTagTuple, typename ThreadIndices, typename ExecObjectList>
struct VTKM_NEVER_EXPORT ExpectedWorkletReturnImpl
{
  ...
  using type = decltype(std::declval<FetchType>().Load(std::declval<ThreadIndices>(), std::declval<ExecObjectType>()));
};
template <typename ControlTagTuple, typename ThreadIndices, typename ExecObjectList>
struct VTKM_NEVER_EXPORT ExpectedWorkletReturnImpl<void, ControlTagTuple, ThreadIndices, ExecObjectList>
{
  using type = void;
};
template <typename WorkletType, typename ThreadIndices, typename... ExecObjects>
using ExpectedWorkletReturn =
  typename ExpectedWorkletReturnImpl<
    SignatureToReturn<ExecutionSignature<WorkletType>>,
...
struct VTKM_NEVER_EXPORT CallWorklet
{
  ...
  VTKM_EXEC inline void StoreResult(const ThreadIndices& threadIndices, ExecObjectTypes&&... execObjects) const
  {
    ...
    vtkm::exec::arg::Fetch<FetchTag, typename ExecTag::AspectTag, std::decay_t<decltype(executionObject)>>
      fetch;
...
VTKM_NEVER_EXPORT VTKM_EXEC
void WorkletInvokeFunctor(const WorkletType& worklet,
                          const ThreadIndicesType& threadIndices,
                          Device device,
                          ExecObjTypes&&... execObjects)
{
  using ControlSignature = detail::ControlSignature<WorkletType>;
  using ExecutionSignature = detail::ExecutionSignature<WorkletType>;

  detail::SignatureToTuple<ControlSignature> controlTags;
  ...
  auto extendedControlTags = controlTags.Apply([](auto... controlTags) {
    return vtkm::MakeTuple(detail::ControlTagDeviceFaker{}, controlTags...);
  });
  ...
  auto tagAndWorkletArgs = executionTags.Transform([device, &threadIndices, &execObjects...](auto execTag) {
    using ExecTag = decltype(execTag);
    using FetchTag = typename vtkm::TupleElement<ExecTag::INDEX, decltype(extendedControlTags)>::FetchTag;
    auto&& execArg = detail::GetArg(std::integral_constant<vtkm::IdComponent, ExecTag::INDEX>{},
                                    device,
                                    std::forward<ExecObjTypes>(execObjects)...);
    vtkm::exec::arg::Fetch<FetchTag, typename ExecTag::AspectTag, std::decay_t<decltype(execArg)>> fetch;
  ...
  detail::CallWorklet<WorkletType, detail::ExpectedWorkletReturn<WorkletType, ThreadIndicesType, ExecObjTypes...>>
  ...
  tagAndWorkletArgs.ForEach([device, &threadIndices, &execObjects...](auto& fetchAndArg) {
...
template <typename Worklet, typename Invocation, typename ThreadIndices, vtkm::IdComponent... Indices>
                                      ...
                                      const ThreadIndices& threadIndices)
{
  DoWorkletInvokeFunctor(
        ...
        invocation,
        threadIndices,
        typename vtkmstd::make_integer_sequence<vtkm::IdComponent, Invocation::ParameterInterface::ARITY>{});
