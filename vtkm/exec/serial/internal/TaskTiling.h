...
VTKM_NEVER_EXPORT void TaskTiling1DExecute(void* w, vtkm::Id start, vtkm::Id end)
{
  ...
  for (vtkm::Id index = start; index < end; ++index)
  {
    vtkm::exec::internal::WorkletInvokeFunctor(
          workletInfo->Worklet,
          workletInfo->Worklet.GetThreadIndices(
            index,
...
public:
  ...
  {
    using WorkletInfoType =
      WorkletInvokeInfo<WorkletType, OutToInPortalType, VisitPortalType, ThreadToOutPortalType, Device,
        std::decay_t<ExecutionObjectTypes>...>;

    this->CallInfo = new WorkletInfoType{ worklet, outToInPortal, visitPortal, threadToOutPortal,
      vtkm::MakeTuple(std::forward<ExecutionObjectTypes>(executionObjects)...) };
