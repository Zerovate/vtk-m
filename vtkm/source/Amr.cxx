...
vtkm::cont::AmrDataSet Amr::Execute() const
{
  ...
  vtkm::cont::AmrDataSet amrDataSet(blocksPerLevel);
  ...
  for (unsigned int l = 0; l < blocksPerLevel.size(); l++)
  {
    for (unsigned int b = 0; b < blocksPerLevel.at(l).size(); b++)
    {
      if (this->Dimension == 2)
      {
        amrDataSet.ReplacePartition(blocksPerLevel.at(l).at(b), this->GenerateDataSet<2>(l, b));
      }
      else if (this->Dimension == 3)
      {
        amrDataSet.ReplacePartition(blocksPerLevel.at(l).at(b), this->GenerateDataSet<3>(l, b));
      }
    }
  }

  // Generate helper arrays
  amrDataSet.GenerateParentChildInformation();
  amrDataSet.GenerateGhostType();
  amrDataSet.GenerateIndexArrays();
  return amrDataSet;
}
