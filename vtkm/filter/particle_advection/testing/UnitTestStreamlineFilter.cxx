...
void TestStreamline()
{
  ...
  for (auto& ds : dataSets)
  {
    ...
    vtkm::filter::particle_advection::Streamline streamline;
...
void TestPathline()
{
  ...
  for (int fType = 0; fType < 2; fType++)
  {
    ...
    for (std::size_t i = 0; i < numDS; i++)
    {
      ...
      if (fType == 0)
      {
        vtkm::filter::particle_advection::Pathline filt;
      ...
      {
        vtkm::filter::particle_advection::PathParticle filt;
        filt.SetActiveField(var);
        filt.SetStepSize(stepSize);
        filt.SetNumberOfSteps(numSteps);
        filt.SetSeeds(seedArray);
...
void TestAMRStreamline(bool useSL)
{
  ...
  for (std::size_t d = 0; d < numDS; d++)
  {
    ...
    if (useSL)
    {
      vtkm::filter::particle_advection::Streamline filter;
    ...
    {
      vtkm::filter::particle_advection::ParticleAdvection filter;
      filter.SetStepSize(0.1f);
      filter.SetNumberOfSteps(100000);
      filter.SetSeeds(seedArray);
  ...
  for (std::size_t idx = 0; idx < allPDs.size(); idx++)
  {
    ...
    if (fType == FilterType::STREAMLINE || fType == FilterType::PATHLINE)
    {
      ...
      else
      {
        ...
        vtkm::filter::particle_advection::Pathline pathline;
    ...
    else if (fType == FilterType::PARTICLE_ADVECTION || fType == FilterType::PATH_PARTICLE)
    {
      ...
      {
        vtkm::filter::particle_advection::ParticleAdvection particleAdvection;
      ...
      else
      {
        ...
        vtkm::filter::particle_advection::PathParticle pathParticle;
